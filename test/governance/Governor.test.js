...
async function deployToken(contractName) {
  try {
    return await deployFBContract(contractName, [tokenName, tokenSymbol, tokenName, version]);
  } catch (error) {
    if (error.message == 'incorrect number of arguments to constructor') {
      ...
      return deployFBContract(contractName, [tokenName, tokenSymbol, tokenName]);
    }
    throw error;
  }
}
      ...
      describe(`using ${Token}`, function () {
        ...
        describe('vote with signature', function () {
          ...
          it('votes with a valid EIP-1271 signature', async function () {
            const wallet = await deployFBContract('ERC1271WalletMock', [this.userEOA]);
