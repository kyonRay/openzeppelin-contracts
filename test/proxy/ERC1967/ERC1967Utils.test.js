...
async function fixture() {
  ...
  const utils = await deployFBContract('$ERC1967Utils');
  const v1 = await deployFBContract('DummyImplementation');
  const v2 = await deployFBContract('CallReceiverMock');

  return { admin, anotherAccount, utils, v1, v2 };
}

// FIXME)): FB not support set Slot
if (network.name === 'hardhat') {
  describe('ERC1967Utils', function () {
    ...
    describe('ADMIN_SLOT', function () {
      before('set admin', async function () {
    ...
    describe('BEACON_SLOT', function () {
      before('set beacon', async function () {
        this.beacon = await deployFBContract('UpgradeableBeaconMock', [this.v1]);
      ...
      describe('upgradeBeaconToAndCall', function () {
        it('sets beacon in storage and emits event', async function () {
          const newBeacon = await deployFBContract('UpgradeableBeaconMock', [this.v2]);
        ...
        it("reverts when beacon's implementation does not contain code", async function () {
          const newBeacon = await deployFBContract('UpgradeableBeaconMock', [this.anotherAccount]);
        ...
        describe('reentrant beacon implementation() call', function () {
          it('sees the new beacon implementation', async function () {
            const newBeacon = await deployFBContract('UpgradeableBeaconReentrantMock');
            await expect(this.utils.$upgradeBeaconToAndCall(newBeacon, '0x'))
